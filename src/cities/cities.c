...
struct city* cities_format(char* city_input, const char* cities_string)
{
  ...
  city* city_ptr = malloc(sizeof(city));
  if (!city_ptr) {
  ...
  /* int stop = 0; */
  /* while (citieslen - offset > 1 && stop == 0) */ 
  for (i = 0; i < city_count; i++)
  {
    sscanf(cities_string+offset, " %63[^:]:%15[^:]:%15[^\n]\n", cities[i]->name, cities[i]->lat, cities[i]->lon);

    /*
    offset += strlen(city_ptr->name) + strlen(city_ptr->lat) + strlen(city_ptr->lon)+3;  Set offset to current city string length and two colons+\n */
