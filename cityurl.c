...
char* cityurl (const char* cities, char* url)
{
    ...
    while (citieslen - offset > 1) //printa URL baserat på val
    {
        ...
        {
            snprintf(url, 256, "https://api.open-meteo.com/v1/forecast?latitude=%s&longitude=%s&current_weather=true", lat, lon);
            printf("URL: \"%s\"\r\n", url);
        }
    }
    ...
    {
        snprintf(url, 256, "Staden %s finns inte i listan\n", val);
    }
